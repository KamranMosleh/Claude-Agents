...
const CITIES = ["Amsterdam","Rotterdam","The Hague","Utrecht","Eindhoven","Groningen","Tilburg","Almere","Breda","Nijmegen","Remote (NL)"];
const TYPES   = ["Any","Full-time","Part-time","Remote","Internship","Freelance"];
const LEVELS  = ["Any level","Entry","Mid","Senior","Lead"];

// v9: two modes only — "waterfall" (Indeed → Web fallback) and "web" (Web only)
const SOURCES = [
...
const INDEED_THRESHOLD = 6; // waterfall skips web if Indeed returns this many or more

function postedInstruction(id) {
  return {
    any:    "",
    week:   "IMPORTANT: Only include jobs posted within the last 7 days. Exclude any older listings.",
    "2weeks": "IMPORTANT: Only include jobs posted within the last 14 days. Exclude any older listings.",
    month:  "IMPORTANT: Only include jobs posted within the last 30 days. Exclude any older listings.",
  }[id] || "";
}
...
const MCPS = [
  { id: "indeed", label: "Indeed", url: "https://mcp.indeed.com/claude/mcp", color: "#003A9B", dot: "#B5D4F4" },
];
...
function buildPrompt(query, city, type, level, cvText, source, posted) {
  const loc = city === "Remote (NL)" ? "remote Netherlands" : `${city}, Netherlands`;
  ...
  const cvSec = hasCv
    ? `\n\nCandidate CV (truncated):\n${truncatedCV}\n\nFor each job add a "cvNote" field: 2–3 specific actionable tips to tailor this CV for that role.`
    : "";
  const sourceNote = source === "web"
    ? "Search across LinkedIn, Glassdoor, Werkzoeken.nl, Nationale Vacaturebank, Monsterboard, Intermediair, Jobbird, YoungCapital, and any other relevant Dutch job boards. Also try searching in Dutch (e.g. 'kwaliteitsmedewerker', 'laborant voedingsmiddelen') as many NL listings are in Dutch."
    : "";
  const dateInstruction = postedInstruction(posted);
  return `Search for ${f ? f + " " : ""}"${query}" jobs in ${loc}. ${sourceNote}${cvSec}

Find 5–8 real current listings. If exact matches are scarce, include closely related roles (e.g. for "quality control" also consider "quality assurance", "QA technician", "laborant", "kwaliteitsmedewerker").
${dateInstruction}
...
async function callClaude({ prompt, mcpServer, useWebSearch }) {
  const body = {
    model: MODEL,
    max_tokens: 4000,
    system: "You are a job search assistant. Search for jobs using your available tools. Your ENTIRE response must be a valid JSON array starting with [ and ending with ]. Do not write any text before or after the array.",
...
function srcLabel(id) {
  return { indeed: "Indeed", web: "Web Search", gemini: "Gemini" }[id] || id;
}
function srcColor(id) {
  return { indeed: "#003A9B", web: "#534AB7", gemini: "#1D6F42" }[id] || "#888";
}
function srcDot(id) {
  return { indeed: "#B5D4F4", web: "#CED3FA", gemini: "#9FE1CB" }[id] || "#ddd";
}

function toMarkdown(jobs, query, city, type, level, mode, filter) {
  const visible = filter === "all" ? jobs : jobs.filter(j => j._src === filter);
  const now = new Date();
  const ds  = now.toLocaleDateString("en-NL", { year: "numeric", month: "long", day: "numeric" });
  const f   = [type !== "Any" && type, level !== "Any level" && level].filter(Boolean).join(", ") || "None";
  let md = `# 🌷 NL Job Search Results\n\n| | |\n|---|---|\n| **Search** | ${query} |\n| **Location** | ${city} |\n| **Filters** | ${f} |\n| **Mode** | ${mode} |\n| **Date** | ${ds} |\n| **Results** | ${visible.length} positions |\n\n---\n\n`;
  visible.forEach((job, i) => {
    md += `## ${i + 1}. ${job.title || "Position"}\n\n`;
    if (job.company)                          md += `**Company:** ${job.company}  \n`;
    if (job.location)                         md += `**Location:** ${job.location}  \n`;
    if (job.type && job.type !== "null")      md += `**Type:** ${job.type}  \n`;
    if (job.salary && job.salary !== "null")  md += `**Salary:** ${job.salary}  \n`;
    if (job.isRemote)                         md += `**Remote:** Yes  \n`;
    if (job.postedDate && job.postedDate !== "null") md += `**Posted:** ${job.postedDate}  \n`;
    if (job._src)                             md += `**Source:** ${srcLabel(job._src)}  \n`;
    md += `\n`;
    if (job.description) md += `### Description\n\n${job.description}\n\n`;
    const reqs = (job.requirements || []).filter(r => r && r !== "null");
    if (reqs.length) { md += `### Requirements\n\n`; reqs.forEach(r => md += `- ${r}\n`); md += `\n`; }
    if (job.cvNote)  md += `### ✦ CV Tailoring Tips\n\n${job.cvNote}\n\n`;
    if (job.url && job.url !== "null") md += `### Apply\n\n[Apply for this position →](${job.url})\n\n`;
    md += `---\n\n`;
  });
  md += `*Generated by NL Job Agent · ${ds}*\n`;
  return md;
}
...
function downloadMd(jobs, query, city, type, level, mode, filter) {
  ...
  a.href = url; a.download = `nl-jobs-${slug}-${ct}-${dt}.md`;
...
// v9: updated SrcBadge — "saved by Indeed" label instead of "US only"
function SrcBadge({ id, status, count, error, savedByIndeed }) {
  ...
  const skipLabel = savedByIndeed ? " saved" : "";
  return (
    <div
      title={err && error ? error : (savedByIndeed ? `Web Search skipped — Indeed returned ${count || INDEED_THRESHOLD}+ results` : undefined)}
      style={{
        display: "inline-flex", alignItems: "center", gap: 5,
        padding: "4px 10px", borderRadius: 12, fontSize: 11, fontWeight: 500,
        border: "0.5px solid",
        borderColor: done ? d : loading ? d+"80" : "#e2e8f0",
        background: done ? d+"22" : loading ? d+"11" : "#f8fafc",
        color: done ? c : loading ? c+"aa" : "#94a3b8",
        transition: "all 0.25s", cursor: (err || savedByIndeed) ? "help" : "default",
      }}>
      {srcLabel(id)}
      {loading && <span style={{ width: 10, height: 10, border: `1.5px solid ${c}40`, borderTopColor: c, borderRadius: "50%", animation: "spin 0.8s linear infinite", display: "inline-block" }} />}
      {done  && count != null && <span style={{ opacity: 0.8 }}> {count}</span>}
      {err   && <span title={error} style={{ color: "#ef4444" }}> ✕</span>}
      {skip  && <span style={{ color: "#94a3b8", fontSize: 10 }}>{skipLabel}</span>}
    </div>
  );
}
    ...
    <div
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ margin: "0 0 2px", fontSize: 15, fontWeight: 500, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{job.title || "Position"}</p>
...
// ── Main component ────────────────────────────────────────────────────────────
export default function NLJobAgent() {
  const [key, setKey] = useState(() => {
    try { return localStorage.getItem("nlja_gemini_key") || ""; } catch { return ""; }
  });
  const [cvText, setCvText] = useState(() => {
    try { return localStorage.getItem("nlja_cv_text") || ""; } catch { return ""; }
  });
  ...
  const [webSavedByIndeed, setWebSavedByIndeed] = useState(false); // v9: track waterfall skip
  const [filter,    setFilter]    = useState("all");
  const [saved,     setSaved]     = useState(false);
  // v9: default to waterfall mode
  ...
  useEffect(() => { try { localStorage.setItem("nlja_gemini_key", key); } catch {} }, [key]);
  useEffect(() => { try { localStorage.setItem("nlja_cv_text", cvText); } catch {} }, [cvText]);
  ...
  const handleSearch = async () => {
    ...
    // ── Claude mode ───────────────────────────────────────────────────────────
      ...
      // v9 waterfall mode: Indeed → Web fallback
      if (srcMode === "waterfall") {
        setS("indeed", "loading");
        setS("web",    "skipped"); // shown as pending; updated below
        ...
        // Step 2 — Web only if Indeed didn't satisfy threshold
        if (indeedJobs.length >= INDEED_THRESHOLD) {
          // Indeed was good enough — skip web, mark it as saved
          setS("web", "skipped");
  ...
  // v9: badge list — only indeed + web in Claude mode (no ziprecruiter/dice)
    ...
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", maxWidth: 700, margin: "0 auto", padding: "24px 16px", color: "#0f172a" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 500 }}>NL Job Agent</div>
      ...
      {showKey && (
        <div style={{ background: "#f8fafc", border: "0.5px solid #e2e8f0", borderRadius: 10, padding: 16, marginBottom: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 11, color: "#94a3b8" }}>
            Gemini key from <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" style={{ color: "#6366f1" }}>aistudio.google.com/apikey</a> · Saved in browser storage automatically.
          </p>
        </div>
      )}
        ...
        <div style={{ background: "#f8fafc", border: "0.5px solid #e2e8f0", borderRadius: 10, padding: 16, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
            <span style={{ fontSize: 11, color: "#94a3b8" }}>
              ...
              {cvTruncated && <span style={{ color: "#f59e0b", marginLeft: 4 }}>· trimmed to {CV_CHAR_LIMIT} chars for prompt</span>}
            </span>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
        <input
          ...
          placeholder='Job title or skill  e.g. "Quality Control Technician", "Food Lab"…'
      ...
      {/* ── Source mode (v9: two options only, with inline description) ── */}
      {!using && (
        <div style={{ marginBottom: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 11, color: "#64748b", paddingLeft: 50, lineHeight: 1.6 }}>
            {srcMode === "waterfall" ? (
              <><strong>Indeed → Web:</strong> Indeed runs first. If it returns {INDEED_THRESHOLD}+ jobs the Web Search is skipped, saving tokens. Otherwise Web Search runs as automatic fallback. Requires Indeed to be connected at <a href="https://claude.ai/settings/integrations" target="_blank" rel="noopener noreferrer" style={{ color: "#6366f1" }}>claude.ai/settings/integrations</a>.</>
            ) : (
              <><strong>Web only:</strong> No MCP needed — searches Werkzoeken.nl, Nationale Vacaturebank, LinkedIn NL, Glassdoor, Monsterboard and more. Best choice when Indeed MCP is unavailable or on Claude desktop.</>
            )}
          </p>
        </div>
      )}
      ...
      {loading && (
        <div style={{ textAlign: "center", padding: "44px 0" }}>
          ...
          <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px" }}>
            {using
              ? "Searching via Gemini + Google Search…"
              : srcMode === "waterfall"
                ? "Searching Indeed first — Web Search on standby…"
                : "Searching Dutch job boards via Web…"}
          </p>
          <p style={{ fontSize: 11, color: "#94a3b8", margin: 0 }}>Usually 15–30 seconds</p>
        </div>
      )}
        ...
        <div style={{ background: "#fef2f2", border: "0.5px solid #fecaca", borderRadius: 8, padding: "12px 16px", fontSize: 13, color: "#b91c1c", lineHeight: 1.65 }}>
          ...
          {!using && srcMode === "waterfall" && (
            <p style={{ margin: "8px 0 0", fontSize: 12, color: "#7f1d1d" }}>
              If Indeed shows ✕, re-connect it at{" "}
              ...
              or switch to <strong>Web only</strong> mode above.
            </p>
          )}
        </div>
      )}
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <p style={{ margin: 0, fontSize: 13, color: "#64748b" }}>
                <strong style={{ color: "#0f172a" }}>{visible.length}</strong> position{visible.length !== 1 ? "s" : ""}
                {filter !== "all" ? ` from ${srcLabel(filter)}` : " found"}
                {cvText.trim() ? " · CV tips included" : ""}
              </p>
      ...
      {!loading && !error && searched && jobs.length === 0 && (
        <div style={{ textAlign: "center", padding: "44px 0" }}>
          ...
          <p style={{ fontSize: 12, color: "#94a3b8", margin: "0 0 12px" }}>
            Try a different keyword or city.
            {srcMode === "waterfall" && " Or switch to Web only if Indeed keeps failing."}
          </p>
          {srcMode === "waterfall" && (
      ...
      {!searched && !loading && (
        <div style={{ textAlign: "center", padding: "40px 0 24px" }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 500, margin: "0 0 8px" }}>Find your next role in the Netherlands</p>
          <p style={{ fontSize: 13, color: "#64748b", lineHeight: 1.8, margin: "0 0 18px" }}>
            Default: Indeed first → Web fallback if needed<br />
            Add a Gemini key for Google Search mode · filter by recency
          </p>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
            {[
              "Indeed → Web waterfall",
              "CV & key auto-saved",
              "CV trimmed to 1 500 chars",
              "Save results as .md",
            ].map(t => (
